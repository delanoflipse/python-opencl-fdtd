...
class SimulationGrid:
  ...
  def fill_region(self, w_min=0.0, w_max=float("inf"), h_min=0.0, h_max=float("inf"), d_min=0.0, d_max=float("inf"), geometry_flag=WALL_FLAG, beta=0.5) -> None:
    ...
    fill_region(self.geometry, self.beta, w_min_int, w_max_int,
                h_min_int, h_max_int, d_min_int, d_max_int, geometry_flag, beta)
  ...
  def get_average_spl_for_source(self, location: Tuple[int, int, int]) -> float:
    p_w, p_h, p_d = location
    return get_averge_spl(self.geometry, p_w, p_h, p_d, self.parameters.dx)
...
@njit(parallel=True)
def fill_region(geometry: np.ndarray, betas: np.ndarray,  w_min_int: int, w_max_int: int,  h_min_int: int, h_max_int: int, d_min_int: int, d_max_int: int, geometry_flag=WALL_FLAG, beta=0.5) -> None:
  """Fill in a region with a flag and set beta"""
  set_beta = geometry_flag & WALL_FLAG > 0
  for d in prange(d_min_int, d_max_int):
    for h in prange(h_min_int, h_max_int):
      for w in prange(w_min_int, w_max_int):
        geometry[w, h, d] |= geometry_flag
        if set_beta:
          betas[w, h, d] = beta


@njit(parallel=True)
def get_averge_spl(geometry: np.ndarray, source_w: int, source_h: int, source_d: int, dx: float) -> float:
